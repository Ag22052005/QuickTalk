  ...
  try {
    ...
    const [friend, senderData] = await Promise.all([
      User.findOne({ phoneNumber: contactNumber })
        ...
        .lean(),
      User.findById(senderObjectId).select("_id contacts name"),
    ]);
        ...
        friend._id,
        {
          $push: {
            contacts: {
              ...
              contactName: senderData.phoneNumber,
            },
          },
        },
        { new: true }
    ...
    console.log("updated friend", updatedUser);
...
const signUp = async (req, res) => {
  try {
    ...
    if (e.errorResponse && e.errorResponse.code === 11000) {
      ...
      res.status(500).json({ status: "Internal-server-Error", error: e });
    }
  }
};
...
const getContact = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ contacts: isUser.contacts });
  } catch (error) {
