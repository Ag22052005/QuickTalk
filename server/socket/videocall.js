const registerVideoCallHandler = async(socket,userSocketMap,io)=>{
  socket.on("video-call-init",async({callerId,roomId,receiverId,offer})=>{
    ...
    socket.to(receiverSocketId).emit("incoming-video-call",{callerId,roomId,receiverId,offer})
  })
  ...
  socket.on("peer:nego:needed",async({receiverId,senderId,offer})=>{
    ...
    socket.to(senderSocketId).emit("peer:negotiating",{receiverId,senderId,offer})
  })
